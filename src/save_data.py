...
def get_connection(credentials_file='mysqlcreds.csv'):     
    ...
    try:
      con = mdb.connect(row['host'], row['user'], row['password'], row['database'], charset='utf8');
    except mdb.Error, e: 
...
def save_vulnerability_data(con, file_id, line_number, code_sample, date_written='', author_name=''):
    vuln_id = execute_query(con, "insert ignore into gh_vuln (file_id, line_number, code_sample, date_written, author_name) values (%s, %s, %s, %s, %s)", \
    (file_id, line_number, code_sample, date_written, author_name));
...
def update_forks_watchers(con, repo_name, username, url, forks_url, stargazers, forks):
    execute_query(con, "update ignore gh_repo set url = %s, forks_url = %s, stargazers = %s, forks = %s where repo_name = %s and username = %s and url is null and forks_url is null and stargazers is null and forks is null", (url, forks_url, stargazers, forks, repo_name, username));
