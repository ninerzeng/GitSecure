...
def parse(result_SA_file_dir, reponame_to_username, result_dict, dump = False ):
	...
	#first grab all files we analyzed
	for f in tree.iter(tag = 'analyzed'):
			key = f.text # this accesses <name> tag
			# structure is [reponame]/[path to file]
			# first find the path to file

			#we want only the c and cpp files
			if not key.endswith(".c") or key.endswith(".cpp"):
				continue 
